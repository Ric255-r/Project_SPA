...
class DetailPaketMassage extends StatefulWidget {
  ...
  DetailPaketMassage({Key? key, required this.idTrans, this.activePromos = const [], this.idMember, required this.namaRoom, required this.statusTamu})
    : super(key: key) {
...
class _DetailPaketMassageState extends State<DetailPaketMassage> {
  ...
  Future<void> getstatusruangan() async {
    try {
      ...
      print(namaroom);
      var response = await dio.get('${myIpAddr()}/ruangan/getstatus', data: {"nama_ruangan": namaroom});
  ...
  Future<void> _storeTrans() async {
    try {
      ...
      Get.dialog(
        AlertDialog(title: Center(child: Text('Loading')), content: Container(width: 100, height: 100, child: Center(child: CircularProgressIndicator()))),
      );
                ...
                TextButton(
                  onPressed: () {
                    ...
                    if (isCash) {
                      ...
                      if (_totalBayarController.text == "" || _totalBayarController.text.isEmpty || int.tryParse(cleanedtotalbayar)! < 0) {
                        return;
                      }
                    }
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(child: Text(formatCurrency.format(hargavip["hargavip"] ?? 0), style: TextStyle(fontFamily: 'Poppins'))),
                          ],
                        ),
                        Row(
                          children: [
